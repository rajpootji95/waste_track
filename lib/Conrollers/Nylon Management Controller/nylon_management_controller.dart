...
class NylonManagementController extends GetxController{
  ...
  Future<GetNylonDetailsAssignedModel?> getNylonDetailsApi(BuildContext context,page,limit,searchText) async {
    ...
    try {
      ...
      debugPrint("this is ${response.requestOptions.uri}");
  ...
  Future<GetSingleVendorModel?> getVendorListApi(BuildContext context,vendorId) async {
    ...
    try {
      ...
      print("reponce====>${response.requestOptions.uri}");
      if (response.statusCode == 200) {
        var result = response.data;

        getSingleVendorModel = GetSingleVendorModel.fromJson(result);
        update();
      await getVendorCollectionApi(context,"1","10","",getSingleVendorModel?.data?.id);
        update();
        if(result['isTokenExpired']??false){
          CustomSnackBar.mySnackBar(
              context, "${result['message']}");
  ...
  Future<GetVendorStaffCollectionListModel?> getVendorCollectionApi(context,page,limit,searchText,vendorId) async {
    ...
    try {
      Response response = await dio.get(
        '${Endpoints.baseUrl}${Endpoints.getCollectionListApi}?page=$page&limit=$limit&searchText=$searchText&requestedVendorId=${vendorId}',
        options: Options(
          headers: headers,
        ),
      );
