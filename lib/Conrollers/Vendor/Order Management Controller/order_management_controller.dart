...
class OrderManagementController extends GetxController {
  ...
  RxNum availableQuantity =  RxNum(0);
  ...
  Future<GetVendorOrdersModel?> getVendorOrdersApi(context,page,limit,searchText) async {
    ...
    try {
      Response response = await dio.get(
        '${Endpoints.baseUrl}${Endpoints.getVendorOrders}?page=$page&limit=$limit&searchText=$searchText',
        options: Options(
          headers: headers,
        ),
      );
      print("qaaaaaaaaaaa${response.requestOptions.uri}");
      if (response.statusCode == 200) {
        dynamic result = response.data;
        getVendorOrdersModel = GetVendorOrdersModel.fromJson(result);
        areOptionVisibleList =
            List.generate(getVendorOrdersModel?.data.length??10, (index) => false).obs;
        update();
        if(getVendorOrdersModel?.status == true){
          debugPrint("${getVendorOrdersModel?.data.length}");
          debugPrint("${result}");
        }
        else{
          CustomSnackBar.mySnackBar(
  ...
  getVendorOrdersDetailsApi(context,vendorId) async {
    ...
    print("aaaaaaaaaaaaaaa${response.requestOptions.uri}");
    if (response.statusCode == 200) {
  ...
  /// order Place
  Future<void> placeOrderApi(num? quantity,int ? paymentStatus, context) async {
    isLoading = true;
    update();
    var userToken = await SharedPref().getToken();
    var token = "${userToken}";
