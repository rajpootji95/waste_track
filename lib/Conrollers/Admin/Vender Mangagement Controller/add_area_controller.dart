...
class AddAreaController extends AppBaseController {
  ...
  Future<void> onInit() async {
    super.onInit();
    await getAreaApi(context ,"1","10","","1");
  ...
  Future<GetAreaModel?> getAreaApi( context,page,limit,searchText,status) async {
    ...
    try {
      Response response = await dio.get(
        '${Endpoints.baseUrl}${Endpoints.getArea}?page=$page&limit=$limit&searchText=$searchText&is_assigned=$status',
        options: Options(
          headers: headers,
        ),
      );
  ...
  /// delete Area Api
  Future<void> deleteAreaApi(String areaId,status) async {
    var userToken = await SharedPref().getToken();
    var token = "${userToken}";
    var headers = {
      'Authorization': 'Bearer $token', // Assuming your token type is 'Bearer'
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              Future.delayed(Duration(milliseconds:0), () {
                getAreaApi(context, "1","10","",status).then(
                        (value){
                      Navigator.pop(context);
                    });
