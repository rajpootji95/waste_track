...
class VendorStaffController extends AppBaseController {
  ...
  /// addVendorStaffApi
  Future<void> addVendorStaffApi(String email, password, firstName, lastName, phoneNumber,addVendorId,context) async {
    isLoading = true;
    update();
    ...
    var requestData = {
      ...
      "vendorId" : addVendorId.toString()
    };
    debugPrint("this is is isi si ${requestData}");
    ...
    try {
      ...
      debugPrint("this Is StatusCode ${response.requestOptions.uri}");
  ...
  Future<GeVendorStaffModel?> getVendorStaffApi(BuildContext context,page,limit,searchText,vendorId) async {
    var userToken = await SharedPref().getToken();
    var token = "${userToken}";
    var headers = {
      'Authorization': 'Bearer $token', // Assuming your token type is 'Bearer'
    ...
    try {
      Response response = await dio.get(
        '${Endpoints.baseUrl}${Endpoints.getVendorStaff}?page=$page&limit=$limit&searchText=${searchText}&requestedVendorId=${vendorId}',
        options: Options(
          headers: headers,
        ),
      );
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            Future.delayed(Duration(milliseconds: 1000), () {
              getVendorStaffApi(context, "1","10","",vendorID);
               Navigator.pop(context);
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              Future.delayed(Duration(milliseconds:0), () {
                getVendorStaffApi(context, "1", "10", "",vendorId).then(
                        (value){
                      Navigator.pop(context);
                    });
