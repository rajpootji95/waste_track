...
class WasteManagementAgencyController extends AppBaseController {
 ...
 /// addWasteAgencyApi
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (result['status'] == true) {
          myShimmer();
          getWasteAgencyApi(context);
          getWasteStaffApi(context, "searchText");
          isLoading = false;
          update();
          firstName == null;
  ...
  Future<GetWasteStaffModel?> getWasteStaffApi(BuildContext context,searchText) async {
    ...
    try {
      Response response = await dio.get(
        '${Endpoints.baseUrl}${Endpoints.getWasteStaff}?page=1&limit=&searchText=$searchText',
        options: Options(
          headers: headers,
        ),
      );
      print("response.requestOptions--->${response.requestOptions.uri}");
      if (response.statusCode == 200) {
        var result = response.data;
