...
import 'Widgets/vendor_add_saff_list.dart';

class AllVendorStaff extends StatefulWidget {
   String? vendorId;
   //GeVendorStaffModel? staffModel;
   AllVendorStaff({super.key,this.vendorId,});
...
class _AllVendorStaffState extends State<AllVendorStaff> {
  ...
  getVendorStaffListApi() async {
   await vendorStaffController.getVendorStaffApi(context, "1", '10','',widget.vendorId);
   setState(() {
   });
  }
  void _onScroll() {
  ...
  Future<void> _fetchData() async {
    ...
    if (!isLoadingMoreData && vendorStaffController.geVendorStaffModel?.data.length != vendorStaffController.geVendorStaffModel?.pagination?.totalItems ) {
      ...
      vendorStaffController.getVendorStaffApi(context, "1", '$limit','',widget.vendorId).then(
              (value){
            setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder(
      init: vendorStaffController,
        builder: (controller) =>
            Scaffold(
              floatingActionButton: FloatingActionButton(
                backgroundColor: AppColor.green,
                onPressed: () async {
                  debugPrint("ddgggdggdgdg ${widget.vendorId}");
                  await  Navigator.push(context, MaterialPageRoute(builder: (context)=>VendorAddStaffList(vendorId: widget.vendorId,))).then((value) async {
                    debugPrint("fdghrhhth ${widget.vendorId}");
                    await vendorStaffController.getVendorStaffApi(context, "1", "", "", widget.vendorId);
                    setState(() {
                    ...
                    children: [
                        ...
                        children: [
                          Flexible(
                            child: CustomSearchTextField(
                              ...
                              onChanged: (value){
                                vendorStaffController.getVendorStaffApi(context, "1", "10", value,widget.vendorId).then((value) {
                                  setState(() {
                      ...
                      Flexible(
                        child: ScrollConfiguration(
                          behavior: MyBehavior(),
                          child:
                          vendorStaffController.geVendorStaffModel == null ? myShimmer(): vendorStaffController.geVendorStaffModel?.data.length == 0 ? Center(child: Text("No vendor staff list !!")) :

                          ListView.builder(
                            controller: _scrollController,
                            itemCount: vendorStaffController.geVendorStaffModel!.data.length,

                            itemBuilder: (context, index) {
                              var staffModelLst = vendorStaffController.geVendorStaffModel!.data[index];
                              print('index=>${index}');
                              return Obx(
                                    () {
                                  return Column(
                                              ...
                                              child: GestureDetector(
                                                onTap: () {
                                                  Navigator.push(
                                                      context,
                                                      MaterialPageRoute(
                                                          builder: (context)=>AllVendorStaffEdit(staffList: vendorStaffController.geVendorStaffModel?.data[index],))
                                                  ).then((value){vendorStaffController.getVendorStaffApi(context,"1","10","",widget.vendorId);});
                                                  setState(() {
                                                            ...
                                                            Row(
                                                              ...
                                                              children: [
                                                                ...
                                                                GestureDetector(
                                                                  onTap: () {
                                                                    Navigator.pop(context);
                                                                    controller.deleteVendorStaffApi(staffModelLst.id ?? "").then((value){vendorStaffController.getVendorStaffApi(context, "1","10", "",widget.vendorId);});
                                                                    setState(() {
